...
/** Status da venda comercial (Sale) → enum Client.statusContrato */
function mapComercialStatusParaStatus(saleStatus) {
  const s = String(saleStatus || "").trim();
  if (s === "Ganho") return "cliente";
  if (s === "Em Negociacao") return "novo_lead";
  if (s === "Perdido") return "novo_lead";
  return null;
}

module.exports = { mapSituacaoParaStatus, mapComercialStatusParaStatus, parseDataBR, parseBlocosCliente };
